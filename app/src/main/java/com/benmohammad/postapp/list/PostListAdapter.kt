...
class PostListAdapter(private val picasso: Picasso):
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int) =  ListViewHolder(LayoutInflater.from(parent.context).inflate(
        R.layout.post_list_item, parent, false), interaction)

    override fun onBindViewHolder(holder: ListViewHolder, position: Int) = holder.bind(getItem(position), picasso)
    ...
    inner class ListViewHolder(
        ...
        fun bind(item: PostWithUser, picasso: Picasso) = with(itemView) {
            ...
            picasso.load(item.getAvatarPhoto()).into(itemView.ivAvatar)
    ...
    private class PostWithUserDC: DiffUtil.ItemCallback<PostWithUser>() {
        override fun areItemsTheSame(oldItem: PostWithUser, newItem: PostWithUser) =
            oldItem.postId == newItem.postId

        override fun areContentsTheSame(oldItem: PostWithUser, newItem: PostWithUser) =
            oldItem == newItem
    }
}